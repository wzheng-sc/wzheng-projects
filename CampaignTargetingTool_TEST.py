...
class CampaignTarget:
    ...
    def validation(self):
        ...
        if not self.has_email_open_constraints:
          ...
          print(f' • Email open constraints: {self.has_email_open_constraints}, {email_open_look_back_days}, Days ')
        ...
        if not self.has_email_bounce_constraints:
          ...
          print(f' • Email bounce constraints: {self.has_email_bounce_constraints}, {email_bounce_look_back_days}, Days ')
    ...
    def query_generator(self):
        ...
        email_open_table_condition = f"""AND email_open._TABLE_SUFFIX between FORMAT_DATE("%y%m%d",DATE_SUB(CURRENT_DATE('America/Los_Angeles'),INTERVAL 2 DAY)) AND FORMAT_DATE("%y%m%d",DATE_SUB(CURRENT_DATE('America/Los_Angeles'),INTERVAL 2 + {self.email_open_look_back_days} DAY))"""
        ...
        email_bounce_table_condition = f"""AND email_bounce._TABLE_SUFFIX between FORMAT_DATE("%y%m%d",DATE_SUB(CURRENT_DATE('America/Los_Angeles'),INTERVAL 2 DAY)) AND FORMAT_DATE("%y%m%d",DATE_SUB(CURRENT_DATE('America/Los_Angeles'),INTERVAL 2 + {self.email_bounce_look_back_days} DAY))"""
