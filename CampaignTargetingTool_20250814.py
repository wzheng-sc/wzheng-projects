...
class CampaignTarget:
    ...
    has_email_bounce_constraints: bool,
    email_bounce_look_back_days: int,
    has_email_open_constraints: bool,
    email_open_look_back_days_upper: int,
    email_open_look_back_days_lower: int,
    has_email_click_constraints: bool,
    email_click_look_back_days_upper: int,
    email_click_look_back_days_lower: int,
    creator_tier: str,
    ...
    region_filters: str,
    is_to_exclude_another_table: bool,
    exclusion_table_name: str,
    is_to_exclude_no_teamsnaps_view_user: bool,
    has_lens_l30_filter: bool,
    lens_l30_value_min: int,
        ...
        self.email_bounce_look_back_days = email_bounce_look_back_days
        self.has_email_bounce_constraints = has_email_bounce_constraints
        self.email_open_look_back_days_lower = email_open_look_back_days_lower
        self.email_open_look_back_days_upper = email_open_look_back_days_upper
        self.has_email_open_constraints = has_email_open_constraints
        self.email_click_look_back_days_lower = email_click_look_back_days_lower
        self.email_click_look_back_days_upper = email_click_look_back_days_upper
        self.has_email_click_constraints = has_email_click_constraints
        ...
        self.has_country_filter = has_country_filter
        self.country_filters = country_filters
        self.has_region_filter = has_region_filter
        self.region_filters = region_filters
        self.is_to_exclude_another_table = is_to_exclude_another_table
        self.exclusion_table_name = exclusion_table_name
        self.is_to_exclude_no_teamsnaps_view_user = is_to_exclude_no_teamsnaps_view_user
        self.has_lens_l30_filter=has_lens_l30_filter
        self.lens_l30_value_min=lens_l30_value_min
    ...
    def validation(self):
        ...
        if self.has_country_filter:
          print(f' • Country: {self.country_filters}')
        else:
          print(f' • Country: All')

        #region
        if self.has_region_filter:
          print(f' • Region: {self.region_filters}')
        else:
          print(f' • Region: All')

        #exclude users without teamsnaps view
        if self.is_to_exclude_no_teamsnaps_view_user:
          print(f' • Exclude Users without TeamSnaps Views in the last 30d')
        else:
          print(f' • No Exclusion based on TeamSnaps Views')

        #lens_l30
        if self.has_lens_l30_filter:
          print(f' • Lens l30 >= {self.lens_l30_value_min}')
        else:
          print(f' • No Lens l30 filter')

        #exclude another table
        if self.is_to_exclude_another_table:
          print(f' • Exclude: {self.exclusion_table_name}')
        else:
          print(f' • No Exclusion Table')
        ...
        #email open
        if not self.has_email_open_constraints:
          print(f' • Email open constraints: {self.has_email_open_constraints}')
        elif self.has_email_open_constraints:
          print(f' • Email open constraints: {self.has_email_open_constraints}, {self.email_open_look_back_days_lower}, ~ , {self.email_open_look_back_days_upper},Days ')

        #email click
        if not self.has_email_click_constraints:
          print(f' • Email click constraints: {self.has_email_click_constraints}')
        elif self.has_email_click_constraints:
          print(f' • Email click constraints: {self.has_email_click_constraints}, {self.email_click_look_back_days_lower}, ~ , {self.email_click_look_back_days_upper},Days ')

        #email bounce
        if not self.has_email_bounce_constraints:
          print(f' • Email bounce constraints: {self.has_email_bounce_constraints}')
        elif self.has_email_bounce_constraints:
          print(f' • Email bounce constraints: {self.has_email_bounce_constraints}, {self.email_bounce_look_back_days}, Days ')
    ...
    def validation_parameters(self):
        ...
        if self.has_age_condition:
            if self.age_from > self.age_to:
                raise ValueError("Error: age's upper bound is smaller than the lower bound.")
    ...
    def query_generator(self):
        ...
        identity_table = "JOIN  `sc-analytics.report_user.identity_20*` AS id ON id.ghost_user_id=uc.ghost_user_id"
        identity_table_condition = """AND id._TABLE_SUFFIX = FORMAT_DATE("%y%m%d",DATE_SUB(CURRENT_DATE('America/Los_Angeles'),INTERVAL 2 DAY))"""
        last_activity_table = "JOIN `sc-analytics.report_app.last_active_day_20*` AS la ON la.ghost_user_id=uc.ghost_user_id"
        ...
        lens_table = "LEFT JOIN `sc-analytics.report_lens.lens_engagement_status_v2_20*` AS lens ON uc.ghost_user_id=lens.ghost_user_id"
        lens_table_condition = f"""AND lens._TABLE_SUFFIX = FORMAT_DATE("%y%m%d",DATE_SUB(CURRENT_DATE('America/Los_Angeles'),INTERVAL 2 DAY)) AND lens_action_active_day_l30 >= {self.lens_l30_value_min}"""
        ...
        exclusion_table = f"""LEFT JOIN `{self.exclusion_table_name}` AS exclusion ON exclusion.ghost_user_id = uc.ghost_user_id"""
        region_table = "LEFT JOIN `sc-analytics.report_growth.user_l90_dimension_20*` AS region ON region.ghost_user_id = uc.ghost_user_id"
        region_table_condition = """AND region._TABLE_SUFFIX = FORMAT_DATE("%y%m%d",DATE_SUB(CURRENT_DATE('America/Los_Angeles'),INTERVAL 2 DAY))"""
        email_open_table = "JOIN `sc-portal.quest.notif_email_campaign_user_20*` AS email_open ON email_open.ghost_user_id = uc.ghost_user_id"
        email_open_table_condition = (
            f"AND email_open._TABLE_SUFFIX BETWEEN FORMAT_DATE('%y%m%d', DATE_SUB(CURRENT_DATE('America/Los_Angeles'), INTERVAL {2 + self.email_open_look_back_days_upper} DAY)) "
            f"AND FORMAT_DATE('%y%m%d', DATE_SUB(CURRENT_DATE('America/Los_Angeles'), INTERVAL {2 + self.email_open_look_back_days_lower} DAY)) "
            "AND (email_open.email_campaign_open > 0 OR email_open.email_transactional_open > 0)"
        )        
        email_bounce_table = "LEFT JOIN `sc-portal.quest.notif_email_campaign_user_20*` AS email_bounce ON email_bounce.ghost_user_id = uc.ghost_user_id"
        email_bounce_table_condition = (
            f"AND email_bounce._TABLE_SUFFIX BETWEEN FORMAT_DATE('%y%m%d', DATE_SUB(CURRENT_DATE('America/Los_Angeles'), INTERVAL {2 + self.email_bounce_look_back_days} DAY)) "
            f"AND FORMAT_DATE('%y%m%d', DATE_SUB(CURRENT_DATE('America/Los_Angeles'), INTERVAL 2 DAY)) "
            "AND (email_bounce.email_campaign_bounce > 0 OR "
            "email_bounce.email_campaign_unsubscribe > 0 OR "
            "email_bounce.email_transactional_bounce > 0 OR "
            "email_bounce.email_transactional_unsubscribe > 0)"
        )        
        email_click_table = f"""JOIN
        (
        SELECT
        ghost_user_id,
        table_suffix_string
        FROM
        (
        SELECT ghost_user_id,_TABLE_SUFFIX AS table_suffix_string
        FROM `sc-portal.quest.notif_email_click_user_20*`
        WHERE (email_campaign_shortlink_click>0 or email_transactional_shortlink_click>0)
        GROUP BY ghost_user_id,table_suffix_string

        UNION ALL

        select ghost_user_id,_TABLE_SUFFIX AS table_suffix_string
        FROM `sc-analytics.report_growth.app_application_external_deep_link_user_level_20*`
        where CASE
              WHEN LOWER(deep_link_url) LIKE LOWER('%email%') THEN 'Email'
              WHEN LOWER(deep_link_url) LIKE LOWER('%unread_messages_reminder%') THEN 'Unread messages reminder'
              WHEN LOWER(deep_link_url) LIKE LOWER('%unseen_pending_request_email%') THEN 'Add Friend'
              WHEN LOWER(short_link_url) LIKE LOWER('%DgVX5KpT%') THEN 'Memories'
              WHEN LOWER(short_link_url) LIKE LOWER('%Fgi7mMuE%') THEN 'Memories'
              WHEN LOWER(short_link_url) LIKE LOWER('%GcOzMyLE%') THEN 'Friend Story'
              WHEN LOWER(short_link_url) LIKE LOWER('%6Y6i0zyQ%') THEN 'Add Friend'
              WHEN LOWER(deep_link_url) LIKE LOWER('%DgVX5KpT%') THEN 'Memories'
              WHEN LOWER(deep_link_url) LIKE LOWER('%Fgi7mMuE%') THEN 'Memories'
              WHEN LOWER(deep_link_url) LIKE LOWER('%GcOzMyLE%') THEN 'Friend Story'
              WHEN LOWER(deep_link_url) LIKE LOWER('%6Y6i0zyQ%') THEN 'Add Friend'
              ELSE NULL
          END is not null
        GROUP BY ghost_user_id,table_suffix_string
        )
        ) AS email_click
        ON email_click.ghost_user_id = uc.ghost_user_id"""
        email_click_table_condition = f"""AND email_click.table_suffix_string BETWEEN FORMAT_DATE("%y%m%d",DATE_SUB(CURRENT_DATE('America/Los_Angeles'),INTERVAL 2 + {self.email_click_look_back_days_upper} DAY)) AND FORMAT_DATE("%y%m%d",DATE_SUB(CURRENT_DATE('America/Los_Angeles'),INTERVAL 2 + {self.email_click_look_back_days_lower} DAY))"""
        teamsnaps_view_user_table = f"""LEFT JOIN `sc-notif-campaigns.scheduled_broadcast_targeting.scheduled_broadcast_user_daily_30d_lookback` AS teamsnaps_view ON teamsnaps_view.ghost_user_id = uc.ghost_user_id"""
        teamsnaps_view_user_table_condition = f"""and (teamsnaps_view.push_success_30d>=5 and coalesce(teamsnaps_viewed_30d,0)=0 and coalesce(chat_teamsnaps_viewed_30d,0)=0)"""
        ...
        country_condition = f"""AND ('{self.country_filters}') LIKE concat('%',uc.l_90_country,'%')""" if self.has_country_filter else " "
        region_condition = f"""AND upper('{self.region_filters}') LIKE upper(concat('%',region.l90_region,'%'))"""
        ...
        email_open_condition = f"""AND email_open.ghost_user_id IS NOT NULL""" if self.has_email_open_constraints else " "
        email_click_condition = f"""AND email_click.ghost_user_id IS NOT NULL""" if self.has_email_click_constraints else " "
        email_bounce_condition = f"""AND email_bounce.ghost_user_id IS NULL""" if self.has_email_bounce_constraints else " "
        exclusion_condition = f"""AND exclusion.ghost_user_id IS NULL""" if self.is_to_exclude_another_table else " "
        teamsnaps_view_user_condition = f"""AND teamsnaps_view.ghost_user_id IS NULL""" if self.is_to_exclude_no_teamsnaps_view_user else " "
        lens_condition = f"""AND lens.ghost_user_id IS NOT NULL""" if self.has_lens_l30_filter else " "
        ...
        if not self.has_region_filter:
          region_table = region_table_condition = region_condition = ""

        # teamsnaps_view_user configurations
        if not self.is_to_exclude_no_teamsnaps_view_user:
          teamsnaps_view_user_table = teamsnaps_view_user_table_condition = teamsnaps_view_user_condition= ""

        # exclusion table configurations
        if not self.is_to_exclude_another_table:
          exclusion_table = exclusion_table_condition = ""
        ...
        # Lens configurations
        if not self.has_lens_l30_filter:
          lens_table = lens_table_condition = lens_condition = ""

        # email open configurations
        if not self.has_email_open_constraints:
          email_open_table = email_open_table_condition = email_open_condition = ""

        # email click configurations
        if not self.has_email_click_constraints:
          email_click_table = email_click_table_condition = email_click_condition = ""

        # email bounce configurations
        if not self.has_email_bounce_constraints:
          email_bounce_table = email_bounce_table_condition = email_bounce_condition = ""
        ...
        # country condition
        if not self.has_country_filter:
          country_condition = ""
        ...
        main_query = f"""
            ...
            {email_open_table_condition}
            {email_click_table}
            {email_click_table_condition}
            {email_bounce_table}
            {email_bounce_table_condition}
            {region_table}
            {region_table_condition}
            {exclusion_table}
            {teamsnaps_view_user_table}
            {teamsnaps_view_user_table_condition}
            {lens_table}
            {lens_table_condition}
            ...
            {last_activity_table_condition}
            {community_condition}
            {email_open_condition}
            {email_click_condition}
            {email_bounce_condition}
            {email_verified_condition}
            {l_7_condition}
            {l_90_condition}
        ...
        email_query_view =  f"""
            ...
            WHERE
            1=1
            AND PARSE_DATE('%Y%m%d', _table_suffix) = DATE_SUB(CURRENT_DATE('America/Los_Angeles'),INTERVAL 2 DAY);
