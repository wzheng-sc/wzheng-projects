...
class CampaignTarget:
    ...
    def query_generator(self):
        ...
        email_open_table_condition = f"""AND email_open._TABLE_SUFFIX between FORMAT_DATE("%y%m%d",DATE_SUB(CURRENT_DATE('America/Los_Angeles'),INTERVAL 2 + {self.email_open_look_back_days_upper} DAY)) AND FORMAT_DATE("%y%m%d",DATE_SUB(CURRENT_DATE('America/Los_Angeles'),INTERVAL 2 + {self.email_open_look_back_days_lower} DAY)) AND ( email_campaign_open>0 OR email_transactional_open>0 )"""
        ...
        email_bounce_table_condition = f"""AND email_bounce._TABLE_SUFFIX between FORMAT_DATE("%y%m%d",DATE_SUB(CURRENT_DATE('America/Los_Angeles'),INTERVAL 2 + {self.email_bounce_look_back_days} DAY)) AND FORMAT_DATE("%y%m%d",DATE_SUB(CURRENT_DATE('America/Los_Angeles'),INTERVAL 2 DAY)) AND ( email_campaign_bounce>0 OR email_campaign_unsubscribe>0 OR email_transactional_bounce>0 OR email_transactional_unsubscribe>0 )"""
        email_click_table = f"""JOIN
        (
          select
        ghost_user_id,table_suffix_string
        FROM
        (
